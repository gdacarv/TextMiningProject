...
public class TfIdf {
	...
	public TfIdf(String foldername) {
		...
		if (datafolder.isDirectory()) {
			insertDocument(foldername, datafolder);
		}
		else {
			docSize++;
	...
	public void insertDocument(String foldername, File datafolder) {
		for(File file : datafolder.listFiles())
			if(file.isDirectory())
				insertDocument(file.getAbsolutePath(), file);
			else if(file.getName().toLowerCase().endsWith(".txt")){
				docSize++;
				insertDocument(foldername + "/" + file.getName());
			}
	}
